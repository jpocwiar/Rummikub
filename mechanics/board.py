...
class Board(QGraphicsScene):
    def __init__(self, view, players, save_data = True, parent=None):
        ...
        self.button.setGeometry(1600, 500, 120, 30)
        ...
        self.generate_tiles()
        self.save_to_db()
        self.logger.log('Start of a game')
    ...
    def make_move(self):
        if self.players[self.current_player_index].first_move:
            ...
            if len(self.players[self.current_player_index].tiles) == len(
                ...
                self.logger.error(str(self.players[self.current_player_index].name) + " did not make his first move!")
                ...
                self.logger.log(
                    str(self.players[self.current_player_index].name) + " laid a combination with sum value of " + str(
                        sum_of_tiles))
                self.board_prev = self.board.copy()
                ...
                self.logger.error(
                    self.players[self.current_player_index].name + " laid his first combination with sum value of " + str(
                        sum_of_tiles) + ". First move requires tiles with sum of values >=30!")
                ...
                self.logger.error(
                    self.players[self.current_player_index].name + " did not make a valid first move and time is out!")

                self.draw_tile()
            elif not self.check_move(own_board) and not self.timed_out:
                self.logger.error(
                    self.players[self.current_player_index].name + " did not make a valid first move!")
        else:
            if len(self.players[self.current_player_index].tiles) == len(self.players[self.current_player_index].tiles_prev) and not self.timed_out:
                #print("Musisz wykonać ruch!")
                self.logger.error(str(self.players[self.current_player_index].name) + " did not make a move!")
                ...
                #print("ruch prawidłowy")
                self.logger.log(
                    str(self.players[self.current_player_index].name) + " made a valid move and has " + str(
                        len(self.players[self.current_player_index].tiles)) + " remaining tiles")
                if len(self.players[self.current_player_index].tiles) == 0:
                    self.logger.log("The winner is " + str(self.players[self.current_player_index].name) + "!")
                    self.save_to_db()
                    msg_box = QMessageBox()
                    msg_box.setText("The winner is " + str(self.players[self.current_player_index].name) + "!")
                ...
                self.logger.error(
                    self.players[self.current_player_index].name + " did not make a valid move and his time is out!")
                self.draw_tile()
            elif not self.check_move(self.board) and not self.timed_out:
                self.logger.error(str(self.players[self.current_player_index].name) + " - wrong move!")
    ...
    def draw_tile(self):
        if len(self.tiles) > 0:
            ...
            self.players[self.current_player_index].tiles_prev = self.players[
                self.current_player_index].tiles.copy()
            self.logger.log(str(self.players[self.current_player_index].name) + " drew a tile")
            self.switch_player()
            self.refresh_board()
        else:
            ...
            self.logger.error("No more tiles! Game over!")
            self.logger.log("The winner is " + min_player.name)
            msg_box = QMessageBox()
            msg_box.setText("No more tiles! Game over! \n The winner is " + min_player.name)
            msg_box.setWindowTitle("Message Box")
            msg_box.setStandardButtons(QMessageBox.Ok)
        ...
        if hasattr(self, 'drag_tile') and self.drag_tile is not None:
            ...
            if not self.drag_tile in self.selected_tiles:
                ...
                if self.drag_tile in self.players[self.current_player_index].tiles and row < 10: #z pulpitu na planszę
                    ...
                    self.logger.error(str(self.players[self.current_player_index].name) + " tried to stole a tile that does not belong to him!")
                ...
                for interval, tile in enumerate(self.selected_tiles):
                    ...
                    if tile in self.players[self.current_player_index].tiles and row < 10:
                        ...
                        self.logger.error(str(self.players[
                                                  self.current_player_index].name) + " tried to stole a tile that does not belong to him!")
